...
class OrderSelectApiControllerTest {
	...
	void setUp(WebApplicationContext webApplicationContext,
		...
		ShippingInfo expectedShppingInfo = ShippingInfo.builder()
			.id(1L)
			.memberNo(1L)
			.receiver("홍길동")
			.mainAddress("test")
			...
			.alternativeNumber("010-1234-1234")
			.shippingRequestMsg("test")
	...
	void When_GetOrderWithCustomer_Expect_Status200() throws Exception {
		...
		UserContext.setCurrentUser(new UserDetailsServiceImpl.CustomUserDetails(expectedOrderResponseDTO.getMember()));
		
		// when - then
		ResultActions response = mockMvc.perform(RestDocumentationRequestBuilders.get("/v1/api/orders/{orderId}", orderId)
	...
	void When_GetOrderWithAdmin_Expect_Status200() throws Exception {
		...
		UserContext.setCurrentUser(new UserDetailsServiceImpl.CustomUserDetails(MemberResponseDTO.builder().role(Role.ROLE_ADMIN).build()));
		;
	...
	void When_GetOrderWithCUSTOMER_Expect_Status404() throws Exception {
		...
		UserContext.setCurrentUser(new UserDetailsServiceImpl.CustomUserDetails(MemberResponseDTO.from(expectedOrderWithCUSTOMERUser.getMember())));
	...
	void When_GetOrdersWithCustomer_Expect_Status200()
		...
		UserContext.setCurrentUser(new UserDetailsServiceImpl.CustomUserDetails(memberDTO));
		
		// when - then
		ResultActions response = mockMvc.perform(
				RestDocumentationRequestBuilders.get("/v1/api/orders/users/{userNumber}", userNumber)
	...
	void When_GetOrdersWithCustomer_Pagination_SpecificYear_Expect_Status404() throws Exception {
		...
		UserContext.setCurrentUser(new UserDetailsServiceImpl.CustomUserDetails(MemberResponseDTO.builder().role(Role.ROLE_CUSTOMER).build()));
