...
class ProductServiceTest {
	...
	class CreateProduct {
		...
		@Test
		@Disabled("해당 테스트는 엔티티 테스트에서 이루어져야합니다.")
		...
		void createProductFailDueToInvalidSellDates() {
			try (MockedStatic<Product> productMockedStatic = mockStatic(Product.class)) {
				...
				Product product = Product.builder()
					...
					.sellStartDatetime(invalidSellStartDatetime)
					.sellEndDatetime(invalidSellEndDatetime)
					.displayName("섬유유연제")
					.brand("섬유나라")
	...
	class UpdateProduct {
		...
		void updateProductSuccess() {
			...
			try (MockedStatic<Product> productMockedStatic = mockStatic(Product.class)) {
				Product product = mock(Product.class);
				ProductUpdateDTO productUpdateDTO = ProductUpdateDTO.builder().build();
