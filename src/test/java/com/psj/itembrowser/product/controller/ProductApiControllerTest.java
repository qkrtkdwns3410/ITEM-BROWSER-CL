...
class ProductApiControllerTest {
	...
	void deleteProductTest() throws Exception {
		...
		mockMvc.perform(delete("/v1/api/products/{productId}", productId))
			.andExpect(status().isOk())
			.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
	...
	@Test
	@DisplayName("상품 삭제 API 실패 - 상품 없음")
	void deleteProductNotFoundTest() throws Exception {
		Long productId = 1L;
		
		doThrow(new NotFoundException(ErrorCode.PRODUCT_NOT_FOUND)).when(productService)
		...
		mockMvc.perform(delete("/v1/api/products/{productId}", productId))
			...
			.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
			.andExpect(status().isNotFound())
			.andExpect(jsonPath("$.error").value("Not Found Product"))
			.andDo(MockMvcRestDocumentationWrapper.document(
				...
				ResourceDocumentation.resource(
					ResourceSnippetParameters.builder()
						...
						.responseFields(
							...
							fieldWithPath("path").description("요청된 URI"),
	...
	void getProductTest() throws Exception {
		Long productId = 1L;
		ProductResponseDTO productResponseDTO = ProductResponseDTO.builder().id(productId)
			.name("Test Product").build();
		
		when(productService.getProduct(productId)).thenReturn(productResponseDTO);
		
		mockMvc.perform(get("/v1/api/products/{productId}", productId))
			.andExpect(status().isOk())
			.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
				...
				ResourceDocumentation.resource(
					ResourceSnippetParameters.builder()
						...
						.responseFields(
							...
							fieldWithPath("sellStartDatetime").description("판매 시작 일시"),
							fieldWithPath("sellEndDatetime").description("판매 종료 일시"),
							fieldWithPath("displayName").description("상품 표시 이름"),
							fieldWithPath("brand").description("브랜드"),
							fieldWithPath("deliveryMethod").description("배송 방법"),
							fieldWithPath("deliveryDefaultFee").description("기본 배송료"),
							fieldWithPath("freeShipOverAmount").description("무료 배송 기준 금액"),
							fieldWithPath("returnCenterCode").description("반품 센터 코드"),
							fieldWithPath("productImages").description("이미지 정보"),
							fieldWithPath("deliveryFeeType").description("배송료 유형")
						)
						.build()
				)
			));
	...
	@Test
	@DisplayName("상품 조회 API 실패 - 상품 없음")
